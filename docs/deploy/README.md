...
| 모드 | 트리 | 첫 잡 | 언제 |
|---|---|---|---|
| `provision` | 비어 있음 | **콜드 — 실측 141 s · 291 s**(§9, 2회) | 캐시 비용을 정직하게 재고 싶을 때 |
...
- 재빌드 후 `④` self-test **exit 0**(`report_outcome=pass`), CLI wall **291.13 s**(콜드 캐시,
  같은 GPU 에 다른 텐넌트 1 프로세스 상주). 지배항은 다시 `robot_spawn` **243.67 s**.
  ⇒ **콜드 비용은 하나의 수가 아니다**(141 s / 291 s, 둘 다 콜드·같은 호스트).
- 잡 종료 후 공유 캐시 **0 B / 파일 0 개** — §3-②'' ⚠ 블록이 두 번째로 재현됐다.
