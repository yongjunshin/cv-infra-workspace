...
**+ 이 호스트가 CI 러너도 돌린다면 일곱 번째 키가 필요하다** — `CV_CI_WORKSPACE_ROOT`
(OPTIONAL, `docker/.env.example` 하단). 제어 평면은 **제출된 호스트 경로를 자기 파일시스템에
대고 검증**한다. CI 러너가 체크아웃한 시나리오 디렉토리(= 커스텀 oracle `.py` 가 함께 있는
그 디렉토리)는 위 6개 루트 **어디에도 속하지 않으므로**, 값을 비워 두면 제어 평면이 그 경로를
*"does not exist"* 로 거절한다 — **파일은 호스트에 멀쩡히 있는데도.** 2026-08-20 실 E2E 가
정확히 이 형태로 죽었다(11스텝 중 10개 성공 후 마지막에서 `oracle_plugin_dir … does not exist`).
값 = 러너의 `_work` 루트 같은 **가장 좁은 상위 디렉토리**. 그 트리 전체가 제어 평면에
**읽기 전용**으로 보이게 되니 넓게 잡지 마라. 러너를 안 돌리는 호스트는 **빈 채로 두면 된다**
(마운트가 아예 추가되지 않는다).
...
| 모드 | 트리 | 첫 잡 | 언제 |
|---|---|---|---|
| `provision` | 비어 있음 | **콜드** — self-test 잡: 120 · 141 · 291 s(etri6000, §9 — 하나의 수가 아니다) · **carter 잡: 643.9 s**(RTX 4080, §9-p5c18) | 캐시 비용을 정직하게 재고 싶을 때 |
| `warm` | 씬 클로저까지 채움 | **웜** — self-test 잡: 28 s(etri6000) · **carter 잡: 67.0 / 71.2 / 73.3 s**(RTX 4080, 3회) | 실사용 배포. 워밍 자체가 Isaac 부팅 1회 값을 낸다(실측 **547 s**, §9-p5c18) |

> **`warm` 은 프로젝트 역사상 2026-08-19 에 처음 행사됐다**(그 전 두 배포는 전부 `provision`).
> 같은 호스트·같은 잡·배타 창에서 잰 짝 = **643.9 s → 70.5 s 평균(9.1×)**, 부팅 구간만 보면
> **606.7 s → 27.4 s(22.2×)**. **워밍 1회(547 s)는 첫 잡에서 이미 본전을 넘는다**(잡당 절약 573 s).
> ⚠ **콜드 비용이 어느 위상에 몰리는지는 호스트마다 다르다** — 아래 §9 두 블록을 비교하라.
...
- **`④` 콜드 라운드트립 `exit 0` / CLI wall 141.05 s** (웜 대조군 27.7 s = **5.09×**).
  ...
  **`robot_spawn` 97.87/1.00 ← 콜드 비용의 지배항** · `first_render_frame` 3.25/0.27 ·
  ...
  ⇒ **이 호스트·이 잡에서는 콜드 페널티가 씬 다운로드가 아니라 robot_spawn 에 몰려 있었다.**
  ⚠ **일반 명제로 읽지 마라** — 2026-08-19(RTX 4080, carter 잡)에서는 정확히 반대였다:
  `scene_load` 543.16/8.17(**66.5×, 지배항**) · `robot_spawn` 1.78/1.74(**1.02×, 페널티 없음**).
  같은 코드·같은 워밍 경로다. **어느 위상이 콜드 비용을 무는지는 호스트/잡마다 재야 한다**
  (아래 p5c18 블록).
...
### §9-p5c18 — **`warm` 모드 최초 행사 + 콜드/웜 짝**(2026-08-19, RTX 4080 호스트)

> 증적 `~/cv-infra-p5c18-t1-evidence/`. 채널 = 로컬. 평면 = `:8021`
> (`cv-infra-orchestrator:p5c17-t4-r2` / 러너 `cv-infra-runner:p5c17-t4-r2`).
> 잡 = **carter warehouse goal**(움직이는 잡 — self-test 자명 시나리오가 아니다, G-80).
> **네 런 전부 `verdict=pass`**, 경로 6.02~6.22 m, mp4 1.7~2.0 MB, MCAP 1.1~1.3 MB.

- **배타 창**: 측정 전·중·후 `nvidia-smi` **compute-apps 표에 우리 러너 외 프로세스 0**
  (0.5 s 간격 1,494 표본 전수 — `n_compute_apps ≥ 2` 인 표본 **0개**). 데스크탑
  graphics 텐넌트(Xorg/gnome-shell/cursor, 약 790~860 MiB)는 상주하나 compute 는 비어 있었다.
  p5c16 이 콜드 3수치(120/141/291 s)를 NFR 에 못 쓴 사유가 **이 증거의 부재**였다.
- **콜드 = 파일 0개 트리**(`provision` 상태, 32,768 B): CLI wall **643.86 s** ·
  runner total **630.04 s** · 컨테이너 수신 **732 MB**.
- **워밍 1회**(`warm_cache.sh <root> warm`): wall **547.08 s**, 트리 **0 → 1,760,066,608 B /
  1,424 파일**, 전량 uid 1234(G-15). 내역 = `cache/home/ov` **1.54 GB**(texturecache 860 MB +
  client 680 MB) · `cache/kit` **219.7 MB**(nv_shadercache/GLCache 190.5 MB + shadercache 27.1 MB)
  · **`cache/computecache` 0 B**(아래 ⚠) · `logs` 126 KB · `data`·`documents` 0 B.
  씬 오픈 338.31 s / 클로저 수신 430.2 MB / prim 5,266.
- **웜 3회**: CLI wall **73.26 / 67.03 / 71.17 s**(평균 70.49, 산포 6.23 s = 평균의 8.8 %).
  잡별 캐시 시딩 = 1,759,940,197 B 를 **0.481 / 0.490 / 0.495 s**(`cache-seed` 로그).

| 위상(s) | 콜드 | 웜 w1 | w2 | w3 | 웜 평균 | 배율 |
|---|---|---|---|---|---|---|
| **CLI wall** | **643.86** | 73.26 | 67.03 | 71.17 | **70.49** | **9.13×** |
| runner total | 630.04 | 57.81 | 52.28 | 55.75 | 55.28 | 11.40× |
| **boot_to_mission** | **606.65** | 27.59 | 27.70 | 26.78 | **27.36** | **22.18×** |
| simulation_app_init | 53.17 | 9.23 | 8.67 | 8.90 | 8.93 | 5.95× |
| **scene_load** | **543.16** | 8.11 | 8.85 | 7.54 | **8.17** | **66.51×** ← 지배항 |
| robot_spawn | 1.78 | 1.76 | 1.76 | 1.69 | 1.74 | 1.02× |
| sut_readiness_wait | 6.78 | 6.79 | 6.82 | 7.22 | 6.94 | 0.98× |
| first_render_frame | 2.66 | 2.72 | 2.66 | 2.65 | 2.68 | 0.99× |
| mission | 23.21 | 30.03 | 24.41 | 28.79 | 27.74 | 0.84× |
| 컨테이너 수신(MB) | 732 | 37.9 | 36.3 | 36.9 | 37.0 | 19.77× |

- **읽는 법**: 캐시가 사는 위상은 `scene_load`(+`simulation_app_init`) **둘뿐**이다.
  `sut_readiness_wait`·`first_render_frame`·`mission` 은 배율 ≈1 — 캐시 항이 **아니다**
  (`mission` 이 0.84× 인 것은 nav2 주행 편차이지 콜드 이득이 아니다).
- **본전**: 워밍 **547 s** 1회 vs 잡당 절약 **573 s** ⇒ **첫 잡에서 회수**된다.
- **공유 캐시는 잡으로 안 데워진다(3번째 재현)**: 콜드 잡은 스크래치에
  `kit +580 엔트리/260.9 MB` · `.cache +839/1,540.3 MB` · `ComputeCache +12/6.8 MB` 를
  썼지만 잡 종료 후 공유 트리는 **여전히 0 B / 0 파일**이었다. 웜 잡 3회 뒤에도 공유 트리는
  **1,760,066,608 B / 1,424 파일 무변**(잡은 원본에 절대 쓰지 않는다). 스크래치 루트는
  네 런 뒤 **비어 있다**(stateless, `NFR-EXEC-002`).
- **웜이 실제로 웜인지의 판정축(G-34)**: 6개 마운트 전부 `writable=true`(EROFS 0), 그리고
  웜 잡의 `.cache` 티어가 **`written=false entries_added=0`** — *"잡이 새로 쓸 게 없었다"*.
- ⚠ **워밍이 채우지 않는 두 조각**(관측): ① **CUDA `ComputeCache` 는 0 파일로 남는다** —
  콜드/웜 잡 모두 매번 `+12 엔트리 / 6.8 MB` 를 재생성한다 ② Kit 캐시도 웜 잡에서
  `+3 엔트리 / 41.0 MB` 가 더 붙는다. **가설**(미검증): `warm_scene.py` 는 스테이지를 열고
  `app.update()` 만 펌프하고 **렌더 프로덕트 1프레임을 산출하지 않는다** — M5 §3 D-B/F10 이
  요구한 형태와 다르다. 잔여 비용은 작으므로(웜 부팅이 27.4 s 로 안정) 긴급하진 않다.
- **부수입(결정성)**: 같은 입력·같은 seed 로 네 런의 `path_len_m` 이 전부 다르다
  (6.0249 / 6.2152 / 6.0862 / 6.2128). 움직이는 잡의 대조군이다 — self-test 의
  `1.9078142372702626e-05` 마지막 자리 일치를 결정성 근거로 인용하면 안 되는 이유(G-80).
- **VRAM(참고)**: 러너 PID 피크 **4,389~4,541 MiB**(per-PID `--query-compute-apps`).
  이 배포의 `CV_VRAM_PER_INSTANCE_MB` 는 **3,785** 로 잡혀 있다 — 예산이 실측보다 **작다**.
  `DoD-P5-11`(Resource Budget) 쪽 입력이지 이 절의 결론은 아니다.
