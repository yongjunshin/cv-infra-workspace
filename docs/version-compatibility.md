...
## 다음 이동 — go2 사족보행 지원(2026-09-01 시점, 태그 컷 전)

go2 지원은 **가산·opt-in**이라 세 축 중 **② ③ 은 값이 움직이지 않는다.** ① 만 움직이고,
그것도 **릴리즈 태그를 컷하는 시점**에 움직인다 — 그래서 위 표는 지금 그대로다
(머리말의 *"현존 값만 기입한다"* 규칙: 컷되지 않은 태그를 앞당겨 적지 않는다).
...
- **③ 계약 `apiVersion`** — `cv-infra/v1` 그대로. `SUPPORTED`/`DEPRECATED` 무변경이므로
  위 호환 표의 3·4열도 그대로다.
- **① Action 태그** — 컷 시 위 두 표의 Action 태그 칸을 **그 태그로** 갱신한다(아래 §행 추가
  정책 1). ⚠ **이번 릴리즈는 소비자 접점 YAML을 바꾸지 않는 대신 러너·제어평면 이미지 안의
  코드로 온다** — 태그 이동만으로는 도달하지 않는다(`docs/deploy/plane-sync.md`, G-43).
