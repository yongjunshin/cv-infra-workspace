#!/usr/bin/env bash
# test_gpu_passthrough.sh — GPU-passthrough gate command, emitted verbatim:
...
#
# ⚠ It does NOT prove the host has none. Measured 2026-08-19: BOTH
...
# absence claim has never been true here, and "no host CUDA/Isaac install
# required" does not rest on one. What it rests on is NON-USE: the deploy plane
# holds zero host-toolkit references, the runner's 8 mounts are all cv-infra
# paths, and the container's CUDA major differs from the host's.
#
...
main() {
  ...
  log "GPU-passthrough gate -> docker run --rm --gpus all $img nvidia-smi"
  ...
  if cv_docker run --rm --gpus all "$img" nvidia-smi; then
    log "GPU passthrough PASS (exit 0) — gate command succeeded"
  else
